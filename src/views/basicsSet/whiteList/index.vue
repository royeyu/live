  ...
  <div class="app-container customerList">
    ...
    <div class="appform conbody-flex formWidth">
      <el-form :model="appform" inline label-width="80px">
        ...
        <el-form-item v-if="isSystem()"  label="业务号码">
          <el-input v-model.trim="appform.account" clearable type="text" placeholder="请输入业务号码" />
        </el-form-item>
        <el-form-item label="白名单">
          <el-input v-model.trim="appform.phone" clearable type="text" placeholder="请输入白名单手机号" />
        </el-form-item>
...
<script>
import {blacklistDel, whitelistPage} from '@/api/settings'

const lineTableData = [
  { label: '客户名称', value: 'custName', width: 100, overText: true },
  { label: '业务号码', value: 'custAccount', width: 100, overText: true },
  { label: '创建日期', value: 'createTime', width: 100, overText: true },
  { label: '白名单号', value: 'phone', width: 100, overText: true },
...
export default {
  ...
  methods: {
    delData(row) {
      this.$confirm(`确认把：${row.phone} 从白名单中删除?`, '提示', {
