          ...
          <el-table-column
            ...
            label="操作">
              <download-excel
                ...
                type="xls"
                header="号码导入失败列表"
                :name="`号码导入失败列表_${new Date().getFullYear()}_${new Date().getMonth()+1}_${new Date().getDate()}_${new Date().getTime()}.xls`"
              >导出</download-excel>
          </el-table-column>
        </el-table>
      </div>
...
import { phoneAdd } from '@/api/customer'
  ...
  methods: {
    ...
    subData(data) {
      ...
      for (const dataKey in data) {
        if (dataKey === 'phoneFile' || dataKey === 'attrFile') {
